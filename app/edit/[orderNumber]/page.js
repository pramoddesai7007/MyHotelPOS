...
const EditOrderPage = ({ params, tableId }) => {
      ...
      try {
        if (orderNumber) {
          ...
          const orderResponse = await axios.get(
            `http://103.159.85.246:5000/api/order/get/order/${orderNumber}`
          );
          ...
          const sectionResponse = await axios.get(
            `http://103.159.85.246:5000/api/section/sectionlist/${tableId}`
          );
  ...
  const handleSaveBill = async () => {
    try {
      ...
      await axios.post(`http://103.159.85.246:5000/api/logHistory/log-history`, {
      ...
      const tableResponse = await axios.get(
        `http://103.159.85.246:5000/api/table/tables/${tableId}`
      );
      const tableInfo = tableResponse.data;

      // Construct the API endpoint for updating the order based on order number
      const updateOrderEndpoint = `http://103.159.85.246:5000/api/order/update-order-by-number/${orderNumber}`;
...
`;
  ...
  useEffect(() => {
    axios
      .get("http://103.159.85.246:5000/api/main")
      .then((response) => {
        console.log(response.data);
