...
const StockOutwardTable = () => {
  ...
  const fetchStockOutwardList = async (start, end) => {
    try {
      const response = await axios.get('http://103.159.85.246:5000/api/stockOut/stockOut', {
        params: {
          startDate: start,
          endDate: end,
        },
  ...
  const fetchItems = async () => {
    try {
      ...
      const itemsWithPrice = await Promise.all(response.data.map(async (item) => {
        const purchaseBillsResponse = await axios.get('http://103.159.85.246:5000/api/purchase/purchases', {
          params: {
