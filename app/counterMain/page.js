...
const CounterMainMenuList = () => {
  ...
  const handleViewMainCategory = async (mainCategory) => {
    try {
      const response = await axios.get(
        `http://103.159.85.246:5000/api/main/${mainCategory._id}`
      );
  ...
  const handleDeleteConfirmed = async (menu) => {
    try {
      ...
      const response = await axios.delete(
        `http://103.159.85.246:5000/api/main/${menu._id}`
      );
  ...
  const handleEditSubmit = async () => {
    try {
      ...
      const response = await axios.patch(
        `http://103.159.85.246:5000/api/main/${editedMenu._id}`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
      ...
      const updatedMenusResponse = await axios.get("http://103.159.85.246:5000/api/main");
  ...
  const handlePicturePreview = (mainImage) => {
    const completeImageUrl = `http://103.159.85.246:5000/${mainImage}`;
              ...
              <div className="p-1 text-center">
                ...
                <p>
                  ...
                  {mainCategoryToView?.mainImage ? (
                    <img
                      src={`http://103.159.85.246:5000/${mainCategoryToView.mainImage}`}
                      alt="Main Category"
