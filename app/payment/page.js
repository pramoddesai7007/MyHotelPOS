...
const PaymentModal = ({
  ...
  const fetchMobileNumbers = async () => {
    try {
      const response = await axios.get(
        "http://103.159.85.246:5000/api/customer/mobile-numbers"
      );
    ...
    try {
      ...
      if (formData.mobileNumber) {
        const customerResponse = await axios.get(
          `http://103.159.85.246:5000/api/customer/get-customer-by-mobile/${formData.mobileNumber}`
        );
      ...
      if (formData.mobileNumber && parseFloat(formData.dueAmount) > 0) {
        ...
        if (updatedCreditBalance > 0) {
          await axios.patch(
            `http://103.159.85.246:5000/api/customer/update-credit-balance/${existingCustomer._id}`,
            {
      ...
      const response = await axios.patch(
        `http://103.159.85.246:5000/api/order/update-order-by-id/${orderID}`,
        {
          ...formData,
      ...
      // await axios.put(`http://103.159.85.246:5000/api/table/${tableId}`, {
      //   isCalled: false,
      // });
  ...
  const addNewCustomer = async () => {
    ...
    const newCustomerResponse = await axios.post(
      "http://103.159.85.246:5000/api/customer/customer/list/add-customer",
      {
        customerName: formData.customerName,
        mobileNumber: formData.mobileNumber,
  ...
  const fetchCustomerDetails = async (mobileNumber) => {
    try {
      const response = await axios.get(
        `http://103.159.85.246:5000/api/customer/get-customer-by-mobile/${mobileNumber}`
      );
  ...
  const handlePaymentMethodClick = async () => {
    try {
      ...
      await axios.put(`http://103.159.85.246:5000/api/order/order/update-flag/${orderNumber}`);
