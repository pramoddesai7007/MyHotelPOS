...
const CounterHotelForm = () => {
  ...
  const fetchHotels = async () => {
    try {
      const response = await axios.get('http://103.159.85.246:5000/api/counterHotel/get-all');
  ...
  const handleEdit = async (hotelId) => {
    try {
      ...
      const response = await axios.get(`http://103.159.85.246:5000/api/counterHotel/get/${hotelId}`);
...
const handleEditInputChange = (e) => {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.patch(`http://103.159.85.246:5000/api/counterHotel/edit/${editFormData._id}`, formDataForUpload);
  ...
  //   try {
  //     await axios.delete(`http://103.159.85.246:5000/api/hotel/delete/${hotelId}`);
  ...
  const handleDeleteConfirm = async () => {
    try {
      await axios.delete(`http://103.159.85.246:5000/api/counterHotel/delete/${deleteHotelId}`);
      console.log('Hotel deleted successfully.');
      fetchHotels(); // Refresh the list after deletion
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://103.159.85.246:5000/api/counterHotel/create', formDataForUpload);
              ...
              <tbody>
                {hotels.map((hotel) => (
                  <tr
                    ...
                    {hotel.hotelLogo && <img src={`http://103.159.85.246:5000/${hotel.hotelLogo}`} alt="Hotel Logo" className="w-10 h-10" />}
                    </td>
